// ============================================================
//  OBRIX ERP — Módulo: Buzón Fiscal SAT
//  Archivo: src/pages/contabilidad/BuzonFiscal.jsx
//  Versión: 1.1 | Abril 2026
//  Cambios v1.1:
//    - SyncStatusBadge ahora refleja el estado real de la e.firma
//      usando config.efirma_configurada (booleano de company_efirma)
//      en lugar de sat_sync_activo (que puede estar siempre false).
//    - Badge muestra "e.firma lista" / "Sin e.firma" para orientar
//      al usuario antes de intentar sincronizar.
//    - Sin cambios en lógica de tabs, KPIs ni tabla CFDI.
// ============================================================
...
import {
  ...
  ChevronDown, Settings, Calendar, ShieldCheck, ShieldOff,
} from 'lucide-react';
import {
...
export default function BuzonFiscal() {
  ...
  useEffect(() => { cargarCfdis(); }, [activeTab, page, filtros, rango]);
    ...
    <MainLayout title="📥 Buzón Fiscal SAT">
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {/* FIX v1.1: badge refleja estado real de e.firma */}
          <EfirmaBadge config={config} />
...
function KpisRow({ stats, onTabClick }) {
  const kpis = [
    { label: 'Recibidas',  valor: stats.total_recibidas  ?? 0, monto: stats.monto_recibidas_mxn, icon: Inbox,      color: 'blue',  tabId: 'recibidas'     },
    { label: 'Emitidas',   valor: stats.total_emitidas   ?? 0, monto: stats.monto_emitidas_mxn,  icon: Send,       color: 'teal',  tabId: 'emitidas'      },
    { label: 'REP / Pagos',valor: stats.total_rep        ?? 0, monto: null,                       icon: CreditCard, color: 'amber', tabId: 'rep'           },
    { label: 'Canceladas', valor: stats.total_canceladas ?? 0, monto: null,                       icon: XCircle,    color: 'red',   tabId: 'cancelaciones' },
  ];
      ...
      {kpis.map(kpi => {
        ...
        return (
          <button
            ...
            className={`text-left p-4 rounded-xl border transition-all hover:shadow-md ${colors.bg} ${colors.border}`}
          >
...
/**
 * FIX v1.1 — EfirmaBadge reemplaza a SyncStatusBadge.
 *
 * Antes usaba sat_sync_activo (campo de company_settings que puede
 * estar en false aunque la e.firma exista). Ahora usa
 * efirma_configurada (booleano real de company_efirma.configurada).
 *
 * Esto da al usuario información accionable:
 *   ✅ "e.firma lista"  → puede sincronizar
 *   ⚠️ "Sin e.firma"   → debe ir a Configuración
 */
function EfirmaBadge({ config }) {
  if (!config) return null;
  return config.efirma_configurada ? (
...
function RangoSelector({ rango, onChange }) {
  const opciones = [
    ...
    { label: 'Mes actual',      mesActual: true },
