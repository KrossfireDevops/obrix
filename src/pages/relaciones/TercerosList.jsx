...
const TerceroCard = ({ tercero, onBloquear, onDesbloquear, onRecalcular }) => {
  ...
  return (
    <div
      ...
      onClick={() => navigate(`/relaciones/terceros/${tercero.id}`)}
    >
...
export const TercerosList = () => {
  ...
  // Estado para modal de bloqueo rápido (reemplaza el prompt() nativo)
  const [modalBloqueo, setModalBloqueo] = useState(null) // { id, motivo }

  const handleBloquear = (terceroId) => {
    setModalBloqueo({ id: terceroId, motivo: '' })
  }

  const confirmarBloqueo = async () => {
    if (!modalBloqueo?.motivo?.trim()) {
      toast.error('El motivo del bloqueo es obligatorio'); return
    }
    const { error } = await service.bloquearTercero(modalBloqueo.id, modalBloqueo.motivo)
      ...
      <MainLayout title="🏢 Directorio Fiscal de Terceros">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => navigate('/relaciones/terceros/nuevo')}
          ...
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.45)',
            ...
            <div style={{ background:'#fff', borderRadius:16, padding:24,
              ...
              <p style={{ fontSize:13, color:'#6B7280', margin:'0 0 16px' }}>
                Ingresa el motivo del bloqueo para continuar.
              </p>
              <textarea
                value={modalBloqueo.motivo}
                onChange={e => setModalBloqueo(m => ({ ...m, motivo: e.target.value }))}
                rows={3} placeholder="Motivo del bloqueo..."
                autoFocus
